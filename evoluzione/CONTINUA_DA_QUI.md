...
- **Death screen:** solo **⌂ blu centrato** in basso (**nessun** audio), **stesse dimensioni** degli altri ⌂ (52–54px scala UI); tap ovunque per riprovare. Dopo il salvataggio (account verificato): **TOP 5** tempi (Generale o Pura come l’ultima run), estratto **Serie attuale** (soglie colore come nel profilo) e **missione attiva** (barra + contatore + tempo residuo; run Plus non avanzano le missioni). Ospite / email non verificata: nessun estratto cloud.
