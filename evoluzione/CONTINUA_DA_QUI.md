...
- **Classifica da home** (`view-leaderboard`): solo **⌂ blu centrato** in basso; **nessun** audio. **Profilo:** idem. **Come si gioca** (`view-howto`): solo **⌂ blu centrato** (come classifica); **nessun** audio. **Profilo pubblico** (`profile.html`) e **⌂ nel dock admin** (`admin.html`): stesso stile del ⌂ in-app; **`--ui-scale`** e gap viewport da `viewport-ui-scale.js` (`applyGameViewportChromeVars`, come il `resize` del canvas `400×720`). Prima del JS resta `--ui-scale: 1` in `style.css`.
