...
- **Classifica da home** (`view-leaderboard`): solo **⌂ blu centrato** in basso; **nessun** audio. **Profilo:** idem. **Come si gioca** (`view-howto`): solo **⌂ blu centrato** (come classifica); **nessun** audio. **Profilo pubblico** (`profile.html`) e **⌂ nel dock admin** (`admin.html`): stesso stile e dimensioni del ⌂ in-app (`--ui-scale` default `1` fuori dal canvas).
