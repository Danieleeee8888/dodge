...
- **Classifica:** due tab **Generale** (TOP 15, miglior tempo assoluto; pallino colore del **Premio Plus** usato in quel record se presente) e **Pura** (TOP 15 solo run **senza** premio Plus). Firestore: `leaderboard` / `scores` (+ campo opzionale `prize_used`), `leaderboard_pure` scritta solo dal backend. `applyOptimisticScore` passa anche il premio della run; `saveScore` (fallback) conta come pura. Indice composito `scores`: `prize_used` + `ms` (deploy `firestore.indexes.json`).
