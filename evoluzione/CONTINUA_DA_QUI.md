...
- **Pagine statiche:** `public/privacy.html`, `public/terms.html`, `public/contact.html` — URL puliti `/privacy`, `/termini`, `/contatti` (rewrite in `firebase.json`). Layout: **⌂ blu centrato in basso**, scroll verticale sulla vista (`.legal-page` fixed + `overflow-y: auto`, come admin), `legal-page.js` + `viewport-ui-scale.js`. Info cookie solo in privacy (nessuna pagina cookie separata).
