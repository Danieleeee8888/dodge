...
- **UI livelli:** in `profile` card livello con progresso prossimo livello; in `profile.html` badge pubblico `LV`; in classifica completa (colonna Tempi: `#records-block-times`) pill `LV` accanto al nome; nella colonna Livello pill `LV` + 5 tile colorati con best streak; in death screen strip `nome | LV` con micro-animazione quando `level_gain > 0` (la TOP5 in morte resta senza livello per riga, e la classifica per livello **non** appare alla morte).
- **Hotfix UX temporaneo (2026-05-27):** UI **Missioni/Premi Plus nascosta** senza rimozione logica. Avvio partita diretto (niente picker premio), profilo con sole tab `Dati` + `Stats` (Mix/Premi hidden). Le vecchie tab Generale/Pura nella vista classifica sono state **rimosse** (sostituite da Tempi + Livello affiancate). Toggle in `public/js/game-engine.js`: `ENABLE_PLUS_MISSIONS_UI = false` (riattivabile; controlla solo il `leaderboardKind` della colonna Tempi: `pure` quando false, `general` quando true).
...
- `records-block` (death TOP 5) / `records-block-times` + `records-block-levels` (vista classifica, due colonne affiancate) → classifiche
