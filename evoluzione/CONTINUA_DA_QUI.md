...
- **Admin overview:** KPI e distribuzione soglie solo dall’archivio **`scores`** (`ms`, `createdAt`) in `GET /api/admin/overview`; finestre 24h/7gg solo su documenti con data creazione leggibile. **Top 15** dalla collezione `leaderboard` (vista classifica).
