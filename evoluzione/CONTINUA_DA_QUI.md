...
- **Classifica da home** (`view-leaderboard`): solo **⌂ blu centrato** in basso; **nessun** audio. **Profilo:** idem. **Come si gioca** (`view-howto`): solo **⌂ blu centrato** (come classifica); **nessun** audio. **Profilo pubblico** (`profile.html`): stesso stile del ⌂ in-app; **`--ui-scale`** da `viewport-ui-scale.js`. **`admin.html`:** `admin-page-shell` in **colonna flex** (`main` scrollabile con `flex:1; min-height:0`, barra `flex-shrink:0`) così lo scroll resta affidabile su Chrome mobile; barra inferiore **nera** (stesso `#000` del body) con ⌂ blu + icona profilo come la home.
