...
- **UI livelli:** in `profile` card livello con progresso prossimo livello; in `profile.html` badge pubblico `LV`; in classifica completa (`#records-block-lb`) pill `LV` accanto al nome; in death screen strip `nome | LV` con micro-animazione quando `level_gain > 0` (la TOP5 in morte resta senza livello per riga).
- **Hotfix UX temporaneo (2026-05-27):** UI **Missioni/Premi Plus nascosta** senza rimozione logica. Avvio partita diretto (niente picker premio), profilo con sole tab `Dati` + `Stats` (Mix/Premi hidden), classifica in-app solo tab **Pura** (Generale hidden). Toggle in `public/js/game-engine.js`: `ENABLE_PLUS_MISSIONS_UI = false` (riattivabile).
