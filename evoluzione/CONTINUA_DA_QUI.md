...
- **Classifica da home** (`view-leaderboard`): solo **⌂ blu centrato** in basso; **nessun** audio. **Profilo:** idem. **Come si gioca** (`view-howto`): solo **⌂ blu centrato** (come classifica); **nessun** audio. **Profilo pubblico** (`profile.html`): stesso stile del ⌂ in-app; **`--ui-scale`** da `viewport-ui-scale.js`. **`admin.html`:** scroll **nativo del documento** (classi `page-scroll-root` / `page-scroll-body` come `profile.html`); meta viewport con `user-scalable=no, maximum-scale=1` e `touch-action: pan-y` su html/body (admin) per **disattivare pinch-zoom** — altrimenti su Chrome Android lo scroll richiede due dita. **Niente** `min-height`/flex sulla `admin-page-shell` (semplice flusso verticale, scroll del documento). Barra inferiore `position: fixed` nera, **⌂ a sinistra, profilo a destra** (`justify-content: space-between`); `.admin-scroll-inner` ha padding-bottom ≈ altezza barra + safe-area così l'ultimo contenuto non finisce sotto. **SW** bumpato a `dodge-v17` quando si rifanno modifiche di layout/CSS critiche per invalidare cache PWA.
