...
- `leaderboard.js`: `fetchBothLeaderboards()` carica generale + pura; `fetchLeaderboard('general'|'pure')`; `saveScore` aggiorna il profilo e invoca l’API **`publish-best-from-profile`** (scores + classifiche lato server). Cache separate per tab.
