...
- **Death screen:** solo **⌂ blu centrato** in basso (**nessun** audio), **stesse dimensioni** degli altri ⌂ (52–54px scala UI); tap ovunque per riprovare. Prima della vista morte: **photo finish** sul canvas (~1 s, cursore rosso). Poi, in un unico reveal: **TOP 5** (Generale/Pura), **Serie attuale** e **missione attiva** (run Plus non avanzano le missioni). Serie: `GET /api/player/stats` o fallback pubblico `GET /api/player/stats/:uid` (come `profile.html`). Ospite / email non verificata: nessun estratto cloud.
