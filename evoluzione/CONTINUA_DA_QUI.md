...
- **Classifica da home** (`view-leaderboard`): solo **⌂ blu centrato** in basso; **nessun** audio. **Profilo:** idem. **Come si gioca** (`view-howto`): solo **⌂ blu centrato** (come classifica); **nessun** audio. **Profilo pubblico** (`profile.html`): stesso stile del ⌂ in-app; **`--ui-scale`** da `viewport-ui-scale.js`. **`admin.html`:** scroll **nativo del documento** (classi `page-scroll-root` / `page-scroll-body` come il profilo pubblico); barra inferiore **`position: fixed`** con stesso `#000` del body — sembra parte della cornice — e i due cerchi (⌂ blu + icona profilo viola) come sulla home. `.admin-scroll-inner` ha padding-bottom ≈ altezza barra + safe-area così l'ultimo contenuto non finisce sotto.
