...
- **Classifica da home** (`view-leaderboard`): solo **⌂ blu centrato** in basso; **nessun** audio. **Profilo:** idem. **Come si gioca** (`view-howto`): solo **⌂ blu centrato** (come classifica); **nessun** audio. **Profilo pubblico** (`profile.html`): stesso stile del ⌂ in-app; **`--ui-scale`** da `viewport-ui-scale.js`. **`admin.html`:** struttura **identica a `profile.html`** — `<main class="admin-view">` figlio diretto di `<body class="admin-body page-scroll-body">`, **niente** wrapper interni (no `admin-page-shell`, no `admin-scroll-outer/inner`). Lo scroll è del documento (`page-scroll-root`); meta viewport con `user-scalable=no, maximum-scale=1`. Barra inferiore `position: fixed` nera, **⌂ a sinistra, profilo a destra** (`justify-content: space-between`); `.admin-view` ha padding-bottom ≈ altezza barra + safe-area così l'ultimo contenuto non finisce sotto. **SW** bumpato (`dodge-v19` al momento dell'allineamento) per invalidare cache PWA.
