...
/** Messaggio se /api/missions/activate fallisce (es. server statico senza rewrite su Functions). */
async function formatMissionActivateFailure(res) {
  if (res.status === 409) {
    return 'Non puoi attivare questa missione (slot occupato o cap premi).';
  }
  if (res.status === 401) {
    return 'Sessione non valida o token scaduto: esci e rientra.';
  }
  if (res.status === 404) {
    return 'API non trovata (404). Le missioni usano le Cloud Functions su /api/…: nella cartella evoluzione esegui «firebase emulators:start --only hosting,functions» e apri l’URL indicato dal terminale (es. http://127.0.0.1:5000). Un semplice «python -m http.server» sulla cartella public non espone /api.';
  }
  let extra = '';
  try {
    ...
    if (ct.includes('application/json')) {
      ...
      if (j && typeof j.error === 'string') extra = ` — ${j.error}`;
    }
  } catch (_) {}
  ...
  missionsRoot.querySelectorAll('.js-mission-activate').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!res.ok) {
          alert(await formatMissionActivateFailure(res));
          return;
        }
        await refreshProfileMissionsAndPrizes();
      } catch (_) {
