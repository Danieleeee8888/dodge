...
function getPlayerTag3() {
  const raw = (safeLocalGet('dodge_player_name', '') || '').trim();
  if (!raw) return '???';
  const u = raw.slice(0, 3).toUpperCase();
  return u.length >= 3 ? u : u + '·'.repeat(3 - u.length);
}

function normalizeRecordEntry(item) {
  if (typeof item === 'number' && item > 0) return { ms: Math.floor(item), tag: '···' };
  if (item && typeof item === 'object' && typeof item.ms === 'number' && item.ms > 0) {
    let tag = (item.tag != null ? String(item.tag) : '???').trim().toUpperCase().replace(/\s+/g, '');
    if (!tag) tag = '???';
    tag = tag.slice(0, 3);
    return { ms: Math.floor(item.ms), tag };
  }
  return null;
}
...
function loadRecordsRaw() {
  try {
    let j = localStorage.getItem(RECORDS_KEY);
    if (!j) j = localStorage.getItem(RECORDS_KEY_LEGACY);
    if (!j) return [];
    const arr = JSON.parse(j);
    if (!Array.isArray(arr)) return [];
    return arr.map(normalizeRecordEntry).filter(Boolean);
  } catch (e) { return []; }
}

function getRecords() {
  return loadRecordsRaw().sort((a, b) => b.ms - a.ms).slice(0, 5);
}

function addRecord(ms) {
  const t = Math.floor(ms);
  if (t <= 0) return;
  const tag = getPlayerTag3();
  const next = [...getRecords(), { ms: t, tag }].sort((a, b) => b.ms - a.ms).slice(0, 5);
  safeLocalSet(RECORDS_KEY, JSON.stringify(next));
}

function removeRecordByEntry(ms, tagEnc) {
  const tag = decodeURIComponent(tagEnc || '');
  const msN = Number(ms);
  const arr = loadRecordsRaw().filter((r) => !(r.ms === msN && r.tag === tag));
  safeLocalSet(RECORDS_KEY, JSON.stringify(arr.sort((a, b) => b.ms - a.ms).slice(0, 5)));
...
function renderRecordsInto(el) {
  ...
  let body = '<h2>TOP 5</h2><ol>';
  if (rec.length === 0) body += '<li class="rec-empty">nessun record</li>';
  else {
    rec.forEach((row, i) => {
      const clsTop = i === 0 ? 'rec-r1' : '';
      const cls = `rec-row${clsTop ? ' ' + clsTop : ''}`;
      const te = encodeURIComponent(row.tag);
      body += `<li class="${cls}"><span class="rec-row-main">${i + 1}. <span class="rec-tag">${row.tag}</span> ${fmt(row.ms)}</span><button type="button" class="rec-del js-no-start" data-rec-ms="${row.ms}" data-rec-tag="${te}" aria-label="Elimina record">×</button></li>`;
    });
  }
  body += '</ol>';
  el.innerHTML = body;
}
function bindLeaderboardRowDeleteOnce() {
  if (document.documentElement.dataset.lbDelBound === '1') return;
  document.documentElement.dataset.lbDelBound = '1';
  document.addEventListener('click', (e) => {
    const btn = e.target.closest && e.target.closest('.rec-del');
    if (!btn) return;
    e.stopPropagation();
    e.preventDefault();
    removeRecordByEntry(btn.getAttribute('data-rec-ms'), btn.getAttribute('data-rec-tag'));
  });
}
...
function hydratePlayerNameInput() {
  const el = document.getElementById('playerNameInput');
  if (!el) return;
  el.value = safeLocalGet('dodge_player_name', '') || '';
}
...
function startGame() {
  ...
  if (nameInput) {
    ...
    safeLocalSet('dodge_player_name', rawName.slice(0, 18));
  }
...
function die() {
  ...
  deathUiTimeoutId = setTimeout(() => {
    ...
    screen.classList.add('screen-death');
    screen.innerHTML = `
      <h1 class="death-heading">Pensavi fosse un gioco facile?</h1>
      <p class="death-sub">sopravvissuto ${fmt(diedElapsed)}</p>
      <div id="records-block"></div>
      <p class="tap-hint">Premi ovunque per riprovare</p>`;
